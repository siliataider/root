...
/**
 \class ROOT::Experimental::Internal::ML::RDataLoader
\brief
...
public:
   RDataLoader(const std::vector<ROOT::RDF::RNode> &rdfs,
   ...
   void Activate()
   {
      ...
      fLoadingThread = std::make_unique<std::thread>(&RDataLoader::LoadData, this);
   }
   ...
   void LoadData()
   {
      ...
      while (true) {
         ...
         fLoadingCondition.wait(lock, [&] {
            return !fIsActive || (fTrainingEpochActive && fTrainingClusterIdx < fClusterLoader->GetNumTrainingClusters()) ||
